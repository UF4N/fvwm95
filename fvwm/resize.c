		   ...
		   unsigned long context, char *action, int *Module) 
{
  ...
  /* Already checked this in functions.c, but its here too in case
  ...
  gcv.line_width = tmp_win->boundary_width - 1; /*2;*/
  ...
  XOffset = eventp->xbutton.x_root - dragx;
  YOffset = eventp->xbutton.y_root - dragy;
  ...
  if (ymotion == -1) YOffset = dragHeight - YOffset;
  if (xmotion == -1) XOffset = dragWidth - XOffset;

  /* draw the rubber-band window */
  MoveOutline (Scr.Root,
               dragx - tmp_win->bw + tmp_win->boundary_width / 2, 
               dragy - tmp_win->bw + tmp_win->boundary_width / 2, 
	       dragWidth + 2 * tmp_win->bw - tmp_win->boundary_width,
	       dragHeight + 2 * tmp_win->bw - tmp_win->boundary_width);
...
void DoResize(int x_root, int y_root, FvwmWindow *tmp_win)
{
  ...
  /* resizing called from menu - find the border/corner the pointer
     first time collides with */
  if((xmotion == 0) && (ymotion == 0))
    {
    int ct; /* corner threshold */

    if ((y_root <= origy) || (y_root >= origy + origHeight - 1))
      {
      ct = origWidth >> 2;
      if (y_root <= origy) ymotion = 1;
      if (y_root >= origy + origHeight - 1) ymotion = -1;
      if (x_root <= origx + ct) xmotion = 1;
      if (x_root >= origx + origWidth - ct - 1) xmotion = -1;
      }
    else if ((x_root <= origx) || (x_root >= origx + origWidth - 1))
      {
      ct = origHeight >> 2;
      if (x_root <= origx) xmotion = 1;
      if (x_root >= origx + origWidth - 1) xmotion = -1;
      if (y_root <= origy + ct) ymotion = 1;
      if (y_root >= origy + origHeight - ct - 1) ymotion = -1;
      }
    XOffset = x_root - dragx;
    YOffset = y_root - dragy;
    if (ymotion == -1) YOffset = dragHeight - YOffset;
    if (xmotion == -1) XOffset = dragWidth - XOffset;
    }

  if ((ymotion == 1)&&(y_root < origy+origHeight-1))
    {
      dragy = y_root - YOffset;
      dragHeight = origy + origHeight - y_root + YOffset;
      action = 1;
    }
  else if ((ymotion == -1)&&(y_root > origy))
    {
      dragy = origy;
      dragHeight = 1 + y_root - dragy + YOffset;
      action = 1;
    }
  
  if ((xmotion == 1)&&(x_root < origx + origWidth - 1))
    {
      dragx = x_root - XOffset;
      dragWidth = origx + origWidth - x_root + XOffset;
      action = 1;
    }
  if ((xmotion == -1)&&(x_root > origx))
    {
      dragx = origx;
      dragWidth = 1 + x_root - origx + XOffset;
      action = 1;
    }
